...
export function findBookmarkById(id: string): IBookmark | null {
  const row = getDB().prepare("SELECT * FROM bookmarks WHERE id = ?").get(id) as RawBookmark | undefined;
  return row ? deserialize(row) : null;
}

export function findBookmarkByUrl(url: string): IBookmark | null {
  const row = getDB().prepare("SELECT * FROM bookmarks WHERE url = ?").get(url) as RawBookmark | undefined;
  return row ? deserialize(row) : null;
}

export function findPaginatedBookmarks(skip: number, limit: number) {
  const db = getDB();
  const bookmarks = (db.prepare("SELECT * FROM bookmarks ORDER BY created_at DESC LIMIT ? OFFSET ?").all(limit, skip) as any[]).map(deserialize);
...
export function findAndUpdateBookmark(id: string, update: Partial<IBookmark>): IBookmark | null {
  const fields: string[] = [];
  const values: any[] = [];
...
export function deleteBookmark(id: number): { deletedCount: number } {
