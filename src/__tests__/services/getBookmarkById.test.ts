...
describe("Bookmarks creation", () => {
  ...
  test("It gets the a Bookmark by ID", async () => {
    const result = await getBookmarkById("69fcf6c34eb330810c7f6d8d");

    expect(findByIdMock).toHaveBeenCalledWith("69fcf6c34eb330810c7f6d8d");

    expect(result).toMatchObject({
      bookmark: mockedBookmarks[0],
    });
  });

  test("It fails with the proper error if the ID is not valid", async () => {
    const result = await getBookmarkById("69fcf6c34eb330810c7f6d8ds");
  ...
  test("Sends proper error when there is no Bookmark with that ID", async () => {
    const result = await getBookmarkById("69fcf6c34eb330810c7f6d6d");

    expect(findByIdMock).toHaveBeenCalledWith("69fcf6c34eb330810c7f6d6d");
